...
function search(query, topK = 3) {
  ...
  const scored = skills.map((s) => {
    ...
    const total = (lexNorm * 0.6 + sem * 0.4) + bonus;
    return {
      key: s.key,
...
// 8 大跨领域真实用例测试
const testCases = [
  { label: "1. 容器与运维", query: "Docker 容器镜像构建与多阶段打包优化" },
  { label: "2. 数据库设计", query: "MySQL 数据库设计与建表 DDL 语法检查" },
  { label: "3. 敏感信息扫描", query: "检查代码里有没有泄漏 AWS Key 和密码" },
  { label: "4. 前端架构", query: "React 状态管理与 Next.js 服务端渲染" },
  { label: "5. 接口测试", query: "HTTP RESTful API 自动化接口测试与断言" },
  { label: "6. 数据报表", query: "解析 CSV 文件并计算各列的均值统计指标" },
  { label: "7. 根本原因排查", query: "线上服务突然 502 报警用 5个为什么深入追查根因" },
  { label: "8. Git 提交", query: "分析暂存区代码变动并生成规范的提交信息" },
];

console.log("================================================================================");
console.log("📊 CJK 混合检索引擎全领域评测报告 (Top 3 命中与区分度)");
console.log("================================================================================\n");

for (const t of testCases) {
  console.log(`🔎 【${t.label}】 查询: "${t.query}"`);
  const hits = search(t.query, 3);
  hits.forEach((h, i) => {
    const star = i === 0 ? "🥇" : i === 1 ? "🥈" : "🥉";
    console.log(`   ${star} ${h.key.padEnd(42)} [fit: ${h.score.toString().padEnd(5)} | BM25: ${h.lex} | 向量: ${h.sem}]`);
    console.log(`      ↳ ${h.description.slice(0, 50)}...`);
  });
  console.log("");
}
